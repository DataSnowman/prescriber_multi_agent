...
def main(raw_response: bool = False):
    """
    Interactive Fabric Data Agent Client.
    Prompts the user for questions in a loop until they type 'quit' or 'exit'.
    """
    ...
    try:
        ...
        print("Type 'quit' or 'exit' to stop. Type 'new' to start a new thread.")
        print("="*60)

        thread_name = f"interactive-{uuid.uuid4().hex[:8]}"
        print(f"🧵 Using thread: {thread_name}")
        
        while True:
            print()
            sample = "What are the Top Lisinopril Prescribers in WA by year?"
            question = input(f"❓ Ask a question (Enter for sample, 'quit' to exit): ").strip()
            ...
            if question.lower() == "new":
                thread_name = f"interactive-{uuid.uuid4().hex[:8]}"
                print(f"🧵 Started new thread: {thread_name}")
                continue
